...
class Database:
   ...
   #----------------------------------------#-----------------------------------------#
    ...
    '''
       Pega o feed de um usuario e para qual ferramenta esse feed sera usado
    ''' 
    ...
    def getFeed(self, username):
        ...
        try:
            self.connect()
            self.startCursor()
            query = 'select pk, url, comment_count, like_count from feed where username=%s'  
            self.cursor.execute(query,(username))
    ...
    def updatePoster(self, posts, username):
        #recebe um dicionario com ids de media como chave e suas urls como valores  
        self.connect()
        self.startCursor()
        for media_id in posts.keys():
            try:
                query =  'insert into poster(created_at, scheduled, media_id, username, url, download_name) values(%s, %s,%s,%s,%s,%s)'
                self.cursor.execute(query,(datetime.now().strftime('%y/%m/%d %H:%M:%S'), posts[media_id]['scheduled'],media_id, username, posts[media_id]['url'], str('downloaded/photo' + str(media_id) + '.jpg')))
            except BaseException as error:
                if str(error).startswith('(1062,'):
                    continue
                else:
